...
ssize_t Buffer::ReadFd(int fd, int *save_errno)
{
    ...
    else
    {
        ...
        Append(extra_buffer, n - ReadableBytes());
    }
...
const char* Buffer::Find(const char* str, size_t len) const
{
    const char* start = BeginRead();
    const char* end = BeginRead() + ReadableBytes();
    const char* ret=std::search(start,end,str,str+len);
